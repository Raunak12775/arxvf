...
$-- %%%%%%%%%%%%%%%
\textbf{$it.name.literal$}$if(it.orcid)$~\orcidlink{$it.orcid$}$endif$$for(it.affiliations)$\\\small $it.department$, \small $it.name$\\$if(it.city)$\small $it.city$$if(it.country)$,\ \small $it.country$$endif$$endif$$endfor$\\$if(it.email)$\small \href{mailto:$it.email$}{\texttt{$it.email$}}$endif$